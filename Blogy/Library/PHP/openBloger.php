<?php
	...
	$storeIDs = array();
	$countStories = 0;
	$sql = "SELECT ID, STORYTITLE, STORYLINK, STORYCONTENT FROM stack$blogerSender ORDER BY ID";
	$pick = $conn->query($sql);
	if ($pick->num_rows > 0) {
		while ($row = $pick->fetch_assoc()) {
			$getId = $row["ID"];
		...
		<head>
			...
			<script type='text/javascript'>
				...
				function callBack() {
					...
					loops--;
					if (loops > 0)
						loadStories(\"$storeIDs\", \"1\", 0, \"0\");
				}

				var flag = 0;
			</script>
		</head>
		<body onload='loadStories(\"$storeIDs\", \"1\", 0, \"0\");' onscroll='checkPos()'>
";
