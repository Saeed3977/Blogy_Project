	...
	function buildPost($sender, $post) {		
		...
		if ($postImg != "NULL") {
			...
			if ($parseUrl['host'] == 'www.metacafe.com') {
				...
				$checkIfVideo = end(explode("/", $postImg));
				$checkIfVideo = ".".end(explode(".", $checkIfVideo));
				if (strpos($checkIfVideo, "mp4") || strpos($checkIfVideo, "ogg") || strpos($checkIfVideo, "webm")) {
					if (strpos($checkIfVideo, "mp4")) {
						$type = "mp4";
					}
					else
					if (strpos($checkIfVideo, "ogg")) {
						$type = "ogg";
					}
					else
					if (strpos($checkIfVideo, "webm")) {
						$type = "webm";
					}

					$cmd = "
						<video controls>
							<source src='$postImg' type='video/$type'>
						</video>
					";
				}
				else {
