<?php
	...
	if ($conn->connect_error) {
		...
		if ($conn->query($sql) === TRUE) {
			...
			//Get titles
			$sql = "SELECT STORYTITLE FROM stack$sender";
			$pick = $conn->query($sql);
			if ($pick->num_rows > 0) {
				while ($row = $pick->fetch_assoc()) {
					$getTitle = $row["STORYTITLE"];
					array_push($storeStories, $getTitle);
				}
			}
		}
	}
	...
	$storeStories = implode(",", $storeStories);
			...
			<script>
				...
				function writePost() {
					...
					for (var i = 0; i < stack.length; i++) {
						if (stack[i].replace(/6996/g, ' ') == title) {
							isHere = 1;
						}
					}
		...
		if (strpos($parseType, "mp4") || strpos($parseType, "ogg") || strpos($parseType, "webm")) {
			...
			$buildImg = "
				<div id='imgContainer' onmouseover='$(\"#$countId\").fadeIn(\"fast\");' onmouseleave='$(\"#$countId\").fadeOut(\"fast\");'>
					...
					<div id='$countId' style='display: none;'>
						<div id='imgOptions'>
							<button type='button' class='split' onclick='playVideo(\"$src\", \"$type\")'>View</button>
